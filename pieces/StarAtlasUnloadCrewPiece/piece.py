...
class StarAtlasUnloadCrewPiece(BasePiece):
    ...
    def init_piece(self):
        ...
        self.url_put_unload_crew_fleet = self.read_secrets('URL_PUT_UNLOAD_CREW_FLEET')
    ...
    def piece_function(self, input_data: InputModel):
        ...
        if fleet_status == FleetStatusEnum.StarbaseLoadingBay:
            ...
            self.logger.info(f"Unloading Crew for {input_data.fleet_name} on ({input_data.destination_x}, {input_data.destination_y}), {input_data.amount}")

            url_formated_unload_crew = self.url_put_unload_crew_fleet.format(input_data.destination_x, input_data.destination_y, input_data.fleet_name, input_data.amount)
            res_action1 = self.retry_put_request(url_formated_unload_crew, client_token_loggedin)
            if not(res_action1):
                raise Exception("unload_crew Error") 
                
            self.logger.info(f"Crew unloaded successfully for {input_data.fleet_name} on ({input_data.destination_x}, {input_data.destination_y}), {input_data.amount}")
