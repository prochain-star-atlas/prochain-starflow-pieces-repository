...
class StarAtlasUnloadAmmoPiece(BasePiece):
    ...
    def piece_function(self, input_data: InputModel, workspace_id):
        ...
        if fleet_status == FleetStatusEnum.StarbaseLoadingBay:
            ...
            self.logger.info(f"Unloading Ammo for {input_data.fleet_name} on ({input_data.destination_x}, {input_data.destination_y}), {input_data.amount}")
            url_formated_unload_ammo = self.url_put_unload_ammo.format(input_data.fleet_name, input_data.amount, input_data.destination_x, input_data.destination_y)
            res_action1 = retry_put_request(url_formated_unload_ammo, client_token_loggedin)
            if not(res_action1):
                raise Exception("unload_ammo Error") 
                
            self.logger.info(f"Ammo Unloaded successfully for {input_data.fleet_name} on ({input_data.destination_x}, {input_data.destination_y}), {input_data.amount}")
