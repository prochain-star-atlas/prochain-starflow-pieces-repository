...
class StarAtlasUnloadAllPiece(BasePiece):
    ...
    def piece_function(self, input_data: InputModel):
        ...
        if fleet_status == FleetStatusEnum.StarbaseLoadingBay:
            ...
            if input_data.include_ammo:

                self.logger.info(f"Unloading Ammo for {input_data.fleet_name} on ({input_data.destination_x}, {input_data.destination_y}), {all_cargos[1]}")
                url_formated_unload_ammo = self.url_put_unload_ammo.format(input_data.fleet_name, all_cargos[1], input_data.destination_x, input_data.destination_y)
                res_action1 = retry_put_request(url_formated_unload_ammo, client_token_loggedin)
                if not(res_action1):
                    raise Exception("unload_ammo Error") 
                
                self.logger.info(f"Ammo Unloaded successfully for {input_data.fleet_name} on ({input_data.destination_x}, {input_data.destination_y}), {all_cargos[1]}")
                
                self.logger.info(f"")

            if input_data.include_fuel:

                self.logger.info(f"Unloading Fuel for {input_data.fleet_name} on ({input_data.destination_x}, {input_data.destination_y}), {all_cargos[0]}")
                url_formated_unload_fuel = self.url_put_unload_fuel.format(input_data.fleet_name, all_cargos[0], input_data.destination_x, input_data.destination_y)
                res_action2 = retry_put_request(url_formated_unload_fuel, client_token_loggedin)
                if not(res_action2):
                    raise Exception("unload_fuel Error") 
                    
                self.logger.info(f"Fuel Unloaded successfully for {input_data.fleet_name} on ({input_data.destination_x}, {input_data.destination_y}), {all_cargos[0]}")

                self.logger.info(f"")

            for ac in all_cargos[2]:
                amount_to_unload = ac["tokenAmount"]
                if input_data.keep_one:
                    amount_to_unload = ac["tokenAmount"] - 1
                url_formated_unload_cargo = self.url_put_unload_cargo.format(input_data.fleet_name, ac["tokenMint"], amount_to_unload, input_data.destination_x, input_data.destination_y)
                res_action3 = retry_put_request(url_formated_unload_cargo, client_token_loggedin)
                if not(res_action3):
